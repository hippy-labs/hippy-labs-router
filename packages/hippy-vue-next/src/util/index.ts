...
let beforeLoadStyleHook: CallbackType = (
  declaration: CallbackType,
): CallbackType => declaration;
 ...
 */
let beforeRenderToNativeHook = (
  _el: HippyElement,
  _style: NativeNodeProps,
) => {};
...
export function unicodeToChar(text: string): string {
  return text.replace(/\\u[\dA-F]{4}|\\x[\dA-F]{2}/gi, (match) =>
    String.fromCharCode(parseInt(match.replace(/\\u|\\x/g, ""), 16)),
  );
}
 ...
 */
export function isStyleMatched(
  matchedSelector: NeedToTyped,
  element: HippyElement,
): boolean {
