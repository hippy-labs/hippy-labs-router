...
/**
 * findNotToSkipNode - find out a node that need inserted to native
 * @param nodes
 * @param startIndex
 * @returns {*}
 */
export function findNotToSkipNode(nodes: HippyNode[] = [], startIndex = 0) {
  let targetNode = nodes[startIndex];
  for (let i = startIndex; i < nodes.length; i++) {
    const node = nodes[i];
    if (node?.isNeedInsertToNative) {
      targetNode = node;
      break;
    }
  }
