...
function getEventNode(targetNode: HippyNode): HippyTypes.EventNode {
  ...
  if (eventsAttributes) {
    ...
    Object.keys(eventsAttributes).forEach((key) => {
...
export class HippyNode extends HippyEventTarget {
  ...
  public get index(): number {
    ...
    if (this.parentNode) {
      ...
      const needInsertToNativeChildNodes: HippyNode[] =
        this.parentNode.childNodes.filter((node) => node.isNeedInsertToNative);
  ...
  public insertBefore(rawChild: HippyNode, rawAnchor: HippyNode | null): void {
    ...
    if (child.parentNode && child.parentNode !== this) {
      ...
      throw new Error(
        "Can not insert child, because the child node is already has a different parent",
      );
    }
    ...
    this.insertChildNativeNode(child, {
      refId: notToSkipRefNode.nodeId,
      relativeToRef: RelativeToRefType.BEFORE,
    });
  }
  ...
  public moveChild(rawChild: HippyNode, rawAnchor: HippyNode | null): void {
    ...
    if (anchor.parentNode && anchor.parentNode !== this) {
      ...
      throw new Error(
        "Can not move child, because the anchor node is already has a different parent",
      );
    }
    ...
    this.moveChildNativeNode(child, {
      refId: notToSkipRefNode.nodeId,
      relativeToRef: RelativeToRefType.BEFORE,
    });
  }
  ...
  public insertChildNativeNode(
    ...
    if (renderRootNodeCondition || renderOtherNodeCondition) {
      ...
      renderInsertChildNativeNode(
        parentNode.convertToNativeNodes(true, refInfo),
      );
  ...
  public updateNativeNode(isIncludeChildren = false): void {
    ...
    // get native nodes
    const updateNodes: convertToNativeNodesReturnedVal =
      this.convertToNativeNodes(isIncludeChildren, {});
  ...
  public convertToNativeNodes(
    ...
    if (isIncludeChild) {
      ...
      this.eachNode((targetNode: HippyNode) => {
        const [nativeChildNodes, eventChildNodes, printedChildNodes] =
          targetNode.convertToNativeNodes(false, refInfo);
      ...
      if (eventNode && Array.isArray(eventNode.eventList)) {
        eventNode.eventList.forEach((eventListItem) => {
          ...
          type === EventHandlerType.ADD &&
            Object.assign(listenerProp, { [name]: listener });
        });
      }
