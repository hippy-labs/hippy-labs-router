...
export interface NativeApiType {
  ...
  getBoundingClientRect: (
    el: HippyNode,
    options?: { relToContainer?: boolean },
  ) => Promise<DOMRect>;
...
export const measureInWindowByMethod = async (
  ...
  const { nodeId } = el;
  trace(...LOG_TYPE, "callUIFunction", {
    nodeId,
    funcName: method,
    params: [],
  });
  return new Promise((resolve) =>
    hippyNativeDocument.callUIFunction(
      nodeId,
      method,
      [],
...
export const Native: NativeApiType = {
  ...
  getBoundingClientRect(el, options): Promise<DOMRect> {
    ...
    return new Promise((resolve, reject) => {
      ...
      trace(...LOG_TYPE, "UIManagerModule", {
        nodeId,
      ...
      hippyNativeDocument.callUIFunction(
        ...
        (res) => {
          if (!res || res.errMsg) {
            return reject(
              new Error(
                res?.errMsg || "getBoundingClientRect error with no response",
              ),
            );
          }
  ...
  parseColor(color: string | number): number {
    ...
    const cache =
      CACHE.COLOR_PARSER ?? (CACHE.COLOR_PARSER = Object.create(null));
    if (!cache[color]) {
      // cache parse result
