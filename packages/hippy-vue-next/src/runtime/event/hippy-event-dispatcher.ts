...
function isTouchEvent(eventName) {
  return (
    ["onTouchDown", "onTouchMove", "onTouchEnd", "onTouchCancel"].indexOf(
      eventName,
    ) >= 0
  );
}
  ...
  receiveComponentEvent(
    ...
    if (!nativeEvent || !domEvent) {
      warn(
        ...LOG_TYPE,
        "receiveComponentEvent",
        "nativeEvent or domEvent not exist",
      );
      return;
    }
    ...
    if (!currentTargetNode || !targetNode) {
      warn(
        ...LOG_TYPE,
        "receiveComponentEvent",
        "currentTargetNode or targetNode not exist",
      );
      return;
    }
    ...
    try {
      if (
        [DOMEventPhase.AT_TARGET, DOMEventPhase.BUBBLING_PHASE].indexOf(
          eventPhase,
        ) > -1
      ) {
