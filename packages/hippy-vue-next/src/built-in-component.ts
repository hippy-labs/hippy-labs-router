...
import { mapHippyEvent, convertImageLocalPath, warn } from "./util";
...
const li: ElementComponent = {
  component: {
    ...
    eventNamesMap: mapHippyEvent([["disappear", "onDisappear"]]),
  },
};
    ...
    attributeMaps: {
      href: {
        ...
        propsValue(value: string) {
          if (
            ["//", "http://", "https://"].filter(
              (url) => value.indexOf(url) === 0,
            ).length
          ) {
            warn(
              `href attribute can't apply effect in native with url: ${value}`,
            );
            return "";
          }
          return value;
