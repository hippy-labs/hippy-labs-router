...
export function registerAnimation(vueApp: App): void {
    ...
    Object.keys(actions).forEach((key) => {
      if (Array.isArray(actions[key])) {
        ...
        const animationSetActions = actionSet.map((animationChild) => {
          const { animationId, animation } = createAnimation({
            ...animationChild,
            repeatCount: 0,
          });
        ...
        const { animationId, animation } = createAnimationSet(
          animationSetActions,
  ...
  function getAnimationIds(style) {
    ...
    let animationIds = Object.keys(otherStyles).map(
      (key) => style[key].animationId,
    );
  ...
  vueApp.component("Animation", {
    ...
    watch: {
      ...
      actions() {
        ...
        setTimeout(() => {
          const actionsDidUpdateFunc =
            this.$attrs[getNormalizeEventName("actionsDidUpdate")];
      ...
      removeAnimationEvent() {
        this.animationIds.forEach((animationId) => {
          ...
          Object.keys(this.animationEventMap).forEach((key) => {
            if (typeof this.$attrs[getNormalizeEventName(key)] !== "function")
              return;
