...
describe("runtime/native.ts", () => {
  ...
  it("test native bridge calls: cookie.getAll invalid", async () => {
    await expect(() => Native.Cookie.getAll("")).toThrow(
      new Error("Native.Cookie.getAll() must have url argument"),
    );
  });

  it("test native bridge calls: cookie.set invalid", async () => {
    expect(() => Native.Cookie.set("", "")).toThrow(
      new Error("Native.Cookie.set() must have url argument"),
    );
  });
  ...
  it("test native bridge calls: cookie", async () => {
    ...
    expect(await Native.Cookie.getAll("https://hippyjs.org")).toEqual(
      "https://hippyjs.org",
    );
  });
  ...
  it("test native bridge calls: imageLoader", async () => {
    ...
    expect(await Native.ImageLoader.getSize("https://hippyjs.org")).toEqual(
      100,
    );
  });
  ...
  it("test parseColor", async () => {
    expect(Native.parseColor("#ffffff", { platform: "ios" })).toEqual(
      4294967295,
    );
  });
  it("test native bridge calls: callUIFunction", async () => {
    ...
    expect(
      Native.callUIFunction(el, "scrollToIndex", [0, 0, true]),
    ).toBeUndefined();
  });
  ...
  it("test native bridge calls: getBoundingClientRect error", async () => {
    ...
    el.isMounted = true;
    await expect(Native.getBoundingClientRect(el)).rejects.toThrow(
      new Error("this view is null"),
    );
  });
  it("test native bridge calls: getBoundingClientRect with no response", async () => {
    const el = new HippyListElement("ul");
    Object.defineProperty(Native.hippyNativeDocument, "callUIFunction", {
      value: (nodeId: number, methodName: string, ...args: NeedToTyped[]) => {
        const [, , callback] = args;
        callback();
      },
    });
    el.isMounted = true;
    await expect(Native.getBoundingClientRect(el)).rejects.toThrow(
      new Error("getBoundingClientRect error with no response"),
    );
  });
  it("test native bridge calls: getBoundingClientRect node not mounted", async () => {
    ...
    await expect(Native.getBoundingClientRect(el)).rejects.toThrow(
      new Error(
        `getBoundingClientRect cannot get nodeId of ${el} or ${el} is not mounted`,
      ),
    );
  });
