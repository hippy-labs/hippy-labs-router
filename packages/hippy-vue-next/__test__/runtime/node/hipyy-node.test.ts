 ...
 */
import { getNodeById } from "../../../src/util/node";
  ...
  describe("test the node operation methods.", () => {
    ...
    describe("append child node.", () => {
      it("no child to move should throw error", () => {
        ...
        expect(() => node.appendChild(null as unknown as HippyNode)).toThrow(
          Error,
        );
      });
      ...
      it("should update the tree level properties after appending element.", () => {
        ...
        expect(
          childHippyNodePre.parentNode === parentHippyNode &&
            childHippyNodeNext.parentNode === parentHippyNode,
        ).toBeTruthy();
        ...
        expect(
          childHippyNodePre.nextSibling === childHippyNodeNext,
        ).toBeTruthy();
        expect(
          childHippyNodeNext.prevSibling === childHippyNodePre,
        ).toBeTruthy();
      });

      it("hydrate node will precache directly", () => {
        const ssrNode = new HippyNode(NodeType.ElementNode, {
          id: 1001,
          index: 0,
          name: "View",
          props: {},
        });
        const childSsrNode = new HippyNode(NodeType.ElementNode, {
          id: 1002,
          index: 0,
          name: "View",
          props: {},
    ...
    describe("insert before child node.", () => {
      it("no child to insert should throw error", () => {
        const node = new HippyNode(NodeType.ElementNode);
        expect(() =>
          node.insertBefore(null as unknown as HippyNode, null),
        ).toThrow(Error);
      });
      ...
      it("should update the tree level properties after insert node.", () => {
        ...
        // should throw error
        expect(() =>
          parentHippyNode.insertBefore(differentChildNode, childHippyNodeLast),
        ).toThrow();
      });
    });
    ...
    describe("move child node.", () => {
      it("no child to move should throw error", () => {
        const node = new HippyNode(NodeType.ElementNode);
        expect(() =>
          node.moveChild(null as unknown as HippyNode, null),
        ).toThrow(Error);
      });
      ...
      it("invalid parent node should throw error", () => {
        ...
        expect(() => newParentNode.moveChild(childNode, anchorNode)).toThrow(
          Error,
        );
        expect(() => newParentNode.moveChild(childNode, newAnchorNode)).toThrow(
          Error,
        );
      });
      ...
      it("should update the tree level properties after move node.", () => {
        ...
        expect(
          childHippyNodeFirst.prevSibling === childHippyNodeLast,
        ).toBeTruthy();
        expect(
          childHippyNodeLast.nextSibling === childHippyNodeFirst,
        ).toBeTruthy();
    ...
    describe("remove child node.", () => {
      it("no child to remove should throw error", () => {
        const node = new HippyNode(NodeType.ElementNode);
        expect(() => node.removeChild(null as unknown as HippyNode)).toThrow(
          Error,
        );
      });
    ...
    describe("find child nodes that satisfy the condition.", () => {
      ...
      it("should find out the node.", () => {
        ...
        const aimNode = parentHippyNode.findChild(
          (node) => node.nodeId === childHippyNodeLast.nodeId,
        );
        expect(aimNode === childHippyNodeLast).toBeTruthy();
        expect(
          parentHippyNode.findChild(
            (node) => node.nodeId === parentHippyNode.nodeId,
          ),
        ).toEqual(parentHippyNode);
        expect(
          parentHippyNode.findChild((node) => node.nodeId === 0),
        ).toBeNull();
      });
    });
