...
describe("util/index.ts", () => {
  ...
  it("unicodeToChar", async () => {
    ...
    expect(index.unicodeToChar("\u0068\u0065\u006c\u006c\u006f")).toEqual(
      "hello",
    );
  });
  ...
  it("setsAreEqual", async () => {
    expect(
      index.setsAreEqual(new Set([1, 2, 3]), new Set([1, 2, 3])),
    ).toBeTruthy();
    expect(
      index.setsAreEqual(new Set(["a", "b", "c"]), new Set(["a", "b", "c"])),
    ).toBeTruthy();
    ...
    expect(
      index.setsAreEqual(new Set([1, 2, 3]), new Set([1, 2, 4])),
    ).toBeFalsy();
  });
  ...
  it("mapEvent should return correct map with given event or event list", () => {
    expect(index.mapHippyEvent("listReady", "initialListReady")).toEqual(
      new Map()
        .set("listReady", "initialListReady")
  ...
  it("convertImageLocalPath should return correct local path", () => {
    expect(
      index.convertImageLocalPath("https://hippjs.org/image/index.png"),
    ).toEqual("https://hippjs.org/image/index.png");
    expect(index.convertImageLocalPath("assets/index.png")).toEqual(
      `${HIPPY_DEBUG_ADDRESS}assets/index.png`,
    );
  });

  it("getEventRedirects should return correct event listener object", () => {
    expect(
      getEventRedirects.call(
        {
          $attrs: {
            dropped: true,
          },
    ...
    expect(
      getEventRedirects.call(
        { $attrs: {} } as unknown as ComponentPublicInstance,
        ["headerReleased"],
      ),
    ).toEqual({});
  });
