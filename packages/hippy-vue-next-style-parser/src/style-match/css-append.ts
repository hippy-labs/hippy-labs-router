...
function polyfillSpecialNodeStyle(
  ...
  if (node.name === "View") {
    ...
    if (
      ...
      typeof nativeBackgroundAndroid.color !== "undefined"
    ) {
      nativeNode.props.nativeBackgroundAndroid.color = translateColor(
  ...
  if (nativeNode?.pId) {
    if (currentScrollViewContainerIdList[nativeNode.pId]) {
      if (currentScrollViewContainerIdList[nativeNode.pId].length > 0) {
        ...
        console.warn(
          "Only one child node is acceptable for View with overflow",
        );
      } else {
        currentScrollViewContainerIdList[nativeNode.pId] = [
          Number(nativeNode.id),
        ];
  ...
  if (nativeNode.name === "TextInput") {
    ["caretColor", "underlineColorAndroid", "placeholderTextColor"].forEach(
      (key) => {
        if (
          nativeNode.props[key] &&
          typeof nativeNode.props[key] !== "number"
        ) {
...
export function insertStyleForSsrNodes(
  ...
  const ssrNodes: StyleNodeList = Object.fromEntries(
    nodeList.map((item) => [item.id, item]),
  );
  // get css match map
  ...
  return nodeList.map((item) => {
    ...
    const style = parseItemStyle(
      getItemHippyCssStyle(matchedSelectors.selectors),
    );
    ...
    item.props.style = Object.assign(
      defaultNativeStyle,
      style,
      item.props.inlineStyle,
    );
