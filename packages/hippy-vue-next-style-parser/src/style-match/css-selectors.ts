...
const getNodeAttrVal = (node, attribute) => {
  ...
  if (
    Array.isArray(node?.styleScopeId) &&
    node?.styleScopeId?.includes(attribute)
  ) {
    return attribute;
  }
};
...
class Selector extends SelectorCore {
  ...
  constructor(selectors: SimpleSelector[]) {
    ...
    this.groups = groups.map(
      (g) => new ChildGroup(g.map((sg) => new SiblingGroup(sg))),
    );
    //               [
    //       ChildGroup:[
    //                    SiblingGroup:[notice, sequence],
  ...
  accumulateChanges(
    ...
    for (let i = 0; i < this.groups.length; i += 1) {
      ...
      } while (node !== bound.right && (node = node.parentNode as StyleNode));
    }
    // 8) 返回 mayMatch（true）
    return mayMatch;
  }
