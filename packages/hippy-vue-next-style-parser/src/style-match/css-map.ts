...
// style load hook
const beforeLoadStyleHook: Function = (declaration: Function): Function =>
  declaration;
...
function createDeclaration(beforeLoadStyle) {
  return (decl) => {
    ...
    if (process.env.NODE_ENV !== "production") {
      if (!newDecl) {
        throw new Error(
          "beforeLoadStyle hook must returns the processed style object",
        );
      }
    }
    return newDecl;
...
function createSelectorFromAst(ast) {
  ...
  for (const currentAst of ast) {
    ...
    const simpleSelectorSequence = createSimpleSelectorSequenceFromAst(
      currentAst[0],
    );
