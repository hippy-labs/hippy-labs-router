...
class SelectorsMap {
  ...
  static removeFromMap(
    ...
    const index = list.findIndex(
      (item) => item.sel.ruleSet.hash === sel.ruleSet?.hash,
    );
  ...
  public static getMatchedNodes(
    selectorId: string,
  ): Set<StyleNode> | undefined {
  ...
  public query(node: StyleNode, ssrNodes?: StyleNodeList): SelectorsMatch {
    ...
    if (props?.attributes) {
      ...
      domClassList = new Set(
        (attributes?.class || "").split(" ").filter((x) => x.trim()),
      );
      domId = attributes.id;
    }
    //------------------------------------------------------------------------
    ...
    selectorsMatch.selectors = selectors
      .filter((sel) =>
        sel.sel.accumulateChanges(node, selectorsMatch, ssrNodes),
      )
  ...
  public removeAsUniversal(sel: SelectorCore): void {
    const index = this.universal.findIndex(
      (item) => item.sel.ruleSet?.hash === sel.ruleSet?.hash,
    );
