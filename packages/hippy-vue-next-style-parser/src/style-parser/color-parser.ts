...
const convertRegExp = (...args: string[]) =>
  `\\(\\s*(${args.join(")\\s*,\\s*(")})\\s*\\)`;
...
const matchers = {
  ...
  hsla: new RegExp(
    `hsla${convertRegExp(NUMBER, PERCENTAGE, PERCENTAGE, NUMBER)}`,
  ),
 ...
 * @public
 */
export function translateColor(color: string | number): number {
