  ...
  it("insertStyleForSsrNodes should work correctly", () => {
    const nodeList = [
      ...
      },
      {
        ...
        index: 0,
        name: "View",
        props: { attributes: { id: "wrapper", class: "wrapperClass" } },
        tagName: "div",
      },
    ...
    const nativeNodes = insertStyleForSsrNodes(
      nodeList as unknown as StyleNode[],
      styleContent,
    );
    // default style and css style check
    expect(nativeNodes[0].props.style).toEqual({
      width: 50,
      backgroundColor: 4286611584,
      height: 50,
      flex: 1,
    });
    expect(nativeNodes[0].props.inlineStyle).toEqual({ flex: 1 });
    expect(nativeNodes[1].props.style).toEqual({
      display: "flex",
      overflowY: "scroll",
    });
    ...
    expect(nativeNodes[5].props.style).toEqual({
      color: 4278190080,
      backgroundImage: "https://hippyjs.org",
      fontWeight: "123",
      width: 24,
    });
    expect(nativeNodes[6].props.style).toEqual({
      position: "absolute",
      top: 0,
      right: 0,
      bottom: 0,
    ...
    expect(nativeNodes[7].props.style).toEqual({
      position: "absolute",
      marginLeft: 10,
    });
    ...
    expect(nativeNodes[9].props.style).toEqual({
      overflowX: "scroll",
      flexDirection: "row",
      textShadowColor: 4286611584,
      textShadowOffset: {
        height: 1,
        width: 1,
        x: 1,
