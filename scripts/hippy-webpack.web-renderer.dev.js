...
module.exports = {
  ...
  resolve: {
    ...
    alias: (() => {
      ...
      const hippyVueRuntimeCorePath = path.resolve(
        __dirname,
        "../../../packages/hippy-vue-next/node_modules/@vue/runtime-core",
      );
      if (fs.existsSync(path.resolve(hippyVueRuntimeCorePath, "index.js"))) {
        console.warn(
          `* Using the @vue/runtime-core in ${hippyVueRuntimeCorePath} as vue alias`,
        );
      ...
      // Remove the section if you don't use it
      const hippyVueNextPath = path.resolve(
        __dirname,
        "../../../packages/hippy-vue-next/dist",
      );
      if (fs.existsSync(path.resolve(hippyVueNextPath, "index.js"))) {
        console.warn(
          `* Using the @hippy/vue-next in ${hippyVueNextPath} as @hippy/vue-next alias`,
        );
      ...
      // Remove the section if you don't use it
      const webRendererPath = path.resolve(
        __dirname,
        "../../../packages/hippy-web-renderer/dist",
      );
      if (fs.existsSync(path.resolve(webRendererPath, "index.js"))) {
        console.warn(
          `* Using the @hippy/web-renderer in ${webRendererPath} as @hippy/web-renderer alias`,
        );
